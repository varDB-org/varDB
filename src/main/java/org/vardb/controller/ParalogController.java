...
import org.vardb.service.ParalogService;
import org.vardb.tool.HtmlTool;

@Controller
public class ParalogController {
    @Autowired
    ParalogService service;

    @ResponseBody
    @RequestMapping( value = "/paralogs", method = RequestMethod.GET)
    ...
    @RequestMapping( value = "/paralog", method = RequestMethod.GET )
    public String view( Model model ) {
    	model.addAttribute( "pages", HtmlTool.getPages() );
    	model.addAttribute( "name", "paralog" );
    	return "paralog";
    }
}
